...
export default function EditBarangayPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosInstance.get(`/barangay/single-barangay/${id}`);
  ...
  const { mutate: updateBarangay, isPending } = useMutation({
    mutationFn: async (data: any) => {
      const res = await axiosInstance.put(
        `/barangay/update-barangay/${id}`,
        data
      );
      return res.data;
    },
  ...
  const {
    data: barangays,
    isPending: isBarangayPending2,
    isError: isBarangayError2,
  } = useQuery({
    queryKey: ["barangay"],
    queryFn: async () => {
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Edit a barangay</CardDescription>
        </CardHeader>
        <CardContent>
